#include "../my_string.h"
my_size_t int_to_wrapper(char *str, Specifiers *data, char *str_for_num,
                          bool is_negative);
...
char *print_int(char *str, Specifiers *data, va_list *args) {
  ...
  my_size_t len = int_to_wrapper(str, data, str_for_num, is_negative);
...
my_size_t int_to_wrapper(char *str, Specifiers *data, char *str_for_num,
  ...
  my_size_t max_len = my_strlen(str_for_num);
  ...
  if (buffer) {
    if (data->accuracy) {
      int signs_to_add = data->accuracy - my_strlen(str_for_num);
    ...
    my_strncat(buffer, str_for_num, max_len);
