...
char* print_float(char* str, Specifiers* data, va_list* args) {
  ...
  char* res = str;
  my_size_t act_len = my_strlen(str_for_f);
  my_strncat(str, str_for_f, act_len);
  free(str_for_f);
  res += act_len;
  return res;
}
...
bool is_nan_or_inf(long double f_arg, char** str, bool* is_negative) {
  ...
  if (f_arg == infinity || f_arg != f_arg) {
    ...
    *str = (char*)calloc(len + 1, sizeof(char));
    (*str)[len] = '\0';
    if (f_arg == infinity)
      my_strncpy(*str, "inf", len);
    else
      my_strncpy(*str, "nan", len);
  }
  return res;
}
...
my_size_t calc_max_strlen_for_fnum(Specifiers* data) {
  const my_size_t max_ull_len =
...
char* make_str_for_float_numb(Specifiers* data, const long double f_arg) {
  const my_size_t max_len = calc_max_strlen_for_fnum(data);
  char* str_for_f =
  ...
  my_size_t accuracy = calc_acc(data);
  ...
  my_size_t len = my_strlen(str_for_f);
  if (accuracy) {
    str_for_f[len++] = '.';
  ...
  my_strncpy(res_str, str_for_f + delete_first, len);
  free(str_for_f);
  return res_str;
}
...
// запятой
my_size_t calc_acc(Specifiers* data) {
  my_size_t accuracy = 6;  // по умолчанию точность = 6
...
void frac_to_str(const long double num, char* str, const my_size_t acc) {
  long double fraction =
      num - (unsigned long long)num;  // только дробная часть числа (целая = 0)
  const my_size_t str_len = my_strlen(str);
...
char* make_str_scientific_notation(Specifiers* data, long double f_arg,
  ...
  {
    str_for_e[0] = '0';
    for (my_size_t i = 1; i <= accuracy; i++) str_for_e[i + 1] = '0';
  }
...
int record_e_for_big_value(Specifiers* data, char* str_for_e, long double f_arg,
                           char** str_for_f) {
  my_size_t accuracy = calc_acc(data);
  my_size_t int_len = my_strcspn(*str_for_f, ".");  // длина целой части
                 ...
                 // целая часть отнимает часть или всю точность у дробной части
  {
    ...
    for (my_size_t i = 1; i <= int_len - 1; i++)
      f_arg /= 10;  // приводим запись к виду *.******
    *str_for_f = make_str_for_float_numb(data, f_arg);
  }
  str_for_e[0] = (*str_for_f)[0];
  for (my_size_t f_ind = 1, e_ind = 2; e_ind <= accuracy + 1;
       f_ind++, e_ind++) {
...
int record_e_for_small_value(char* str_for_e, const char* str_for_f,
  ...
  for (my_size_t e_ind = 2; e_ind <= (accuracy + 1); e_ind++, f_ind++)
    str_for_e[e_ind] = str_for_f[f_ind];
  return exponent;
}
...
void finish_record_sign_notation(const char e_spec, char* str_for_e,
  ...
  buffer[0] = e_spec;
  my_strncat(str_for_e, buffer, 1);
  if (exponent >= 0)
    my_strncat(str_for_e, "+", 1);
  else {
    my_strncat(str_for_e, "-", 1);
    exponent *= -1;
  }
  if (exponent < 10)
    my_strncat(str_for_e, "0", 1);
  else {
...
my_size_t calc_first_zeroes(const long double num) {
  long double fraction =
      num - (unsigned long long)num;  // только дробная часть числа (целая = 0)
  const my_size_t max = 100;
  my_size_t counter = 0;
...
char* g_spec_handler(
  ...
  my_size_t precision = calc_acc(data);

  // запоминаем точность
  bool dot_copy = data->dot;
  data->dot = true;
  my_size_t precision_copy = data->accuracy;
      ...
      (int)precision <= exponent)  // случай экспоненциальной записи
  {
    ...
    {
      my_size_t len = my_strlen(str_for_g);
      my_size_t deleted = remove_zeroes_after_point(str_for_g);
...
my_size_t remove_zeroes_after_point(char* str) {
  my_size_t counter = 0;
  my_size_t len = my_strlen(str);
  my_size_t ind = len - 1;
  my_size_t point_ind = my_strcspn(str, ".");
  bool remove = true;
  while (remove && ind > 0) {
