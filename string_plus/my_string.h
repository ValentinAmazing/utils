...
void* my_memchr(const void* str, int c,
                 my_size_t n);  // Выполняет поиск первого вхождения символа c
                                 // (беззнаковый тип) в первых n байтах строки,
                                 // на которую указывает аргумент str
int my_memcmp(const void* str1, const void* str2,
               my_size_t n);  // Cравнивает первые n байтов str1 и str2
void* my_memcpy(void* dest, const void* src,
                 my_size_t n);  // Копирует n символов из src в dest
void* my_memset(
    void* str, int c,
    my_size_t n);  // Копирует символ c (беззнаковый тип) в первые n символов
                    // строки, на которую указывает аргумент str

char* my_strncat(
    char* dest, const char* src,
    my_size_t n);  // Добавляет строку, на которую указывает src, в конец
...
int my_strncmp(
    ...
    my_size_t n);  // Сравнивает не более первых n байтов str1 и str2
char* my_strncpy(char* dest, const char* src,
                  my_size_t n);  // Копирует до n символов из строки, на
                                  // которую указывает src, в dest

my_size_t my_strcspn(
    const char* str1,
...
my_size_t my_strlen(const char* str);  // Вычисляет длину строки str, не
...
char* my_strtok(char* str, const char* delim);  // Разбивает строку str на ряд
...
typedef struct {
  ...
  my_size_t width;     // Ширина поля для вывода числа
  my_size_t accuracy;  // точность (количество знаков) после запятой.
...
my_size_t scan_int(char** format);
...
void frac_to_str(
    ...
    const my_size_t len);  // функция записи дробной части числа. Передаётся
...
my_size_t calc_max_strlen_for_fnum(
...
int record_e_for_small_value(
    ...
    my_size_t accuracy);  // запись числа в научной нотации в случае с нулевой
...
my_size_t remove_zeroes_after_point(
    char*
...
void fill_to_width(
    ...
    const my_size_t signs_to_add);  // добавляет нужное количество символов
                                     // filler в начало строки
my_size_t wrapper_handler(
