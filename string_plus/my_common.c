...
void int_to_str(char* str, int ind, unsigned long long num) {
  const my_size_t max_size = 22;  // заменить на размер максимального целого
  char buffer[max_size];  // массив для записи числа в обратном порядке
  my_size_t int_len = 0;  // вычисляемая длина целой части
  if (num == 0) {
    buffer[0] = '0';
    int_len = 1;
  } else {
...
char* wrapper(Specifiers* data, char* str_after_spec, bool is_negative) {
  ...
  if (data->minus)  // форматирование по левому краю
  {
    ...
    my_strncat(str_with_format, str_after_spec, act_len);
...
void fill_to_width(char* str, int ind, const char filler,
                   const my_size_t signs_to_add) {
  for (my_size_t i = ind; i < ind + signs_to_add; i++) str[i] = filler;
}

my_size_t wrapper_handler(char* str, Specifiers* data, char* str_after_spec,
                           bool is_negative) {
  const my_size_t maxlen = my_strlen(str_after_spec);
  char work_str[maxlen + 1];
  work_str[maxlen] = 0;
  my_strncpy(work_str, str_after_spec, maxlen);
